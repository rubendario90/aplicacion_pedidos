...
if ($transaccion > 0 && $documento > 0) {
    try {
        ...
        $sql_factura = "UPDATE factura SET estado = 'Entregado' WHERE IntTransaccion = :transaccion AND IntDocumento = :documento";
        ...
        if ($row) {
            $sql_estado = "INSERT INTO estado (factura_id, user_id, estado, fecha, user_name) 
                           VALUES (:factura_id, :user_id, 'Entregado', NOW(), :user_name)";
        ...
        $sql_update_gestionada = "UPDATE factura_gestionada 
                                  SET estado = 'Entregado', user_name = :user_name 
        ...
        echo "<script>
       alert('Estado actualizado a \"Entregado\" en factura y factura gestionada correctamente.');
